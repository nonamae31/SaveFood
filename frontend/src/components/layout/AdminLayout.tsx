...
import { Users, Store, LogOut, Menu, X, Shield, ChevronRight, ChevronLeft, CreditCard, LayoutGrid, ShieldCheck, AlertCircle } from 'lucide-react';
...
export function AdminLayout() {
  ...
  const navItems = [
    ...
    { name: 'Khiếu nại', href: ROUTES.ADMIN_COMPLAINTS, icon: AlertCircle },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed lg:sticky top-0 left-0 z-50 h-screen bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out",
        ...
        <div className="h-16 flex items-center px-6 border-b border-gray-100 relative">
          <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
            ...
            {!isDesktopCollapsed && (
              <span className="text-xl font-bold font-display text-gray-900 tracking-tight transition-opacity duration-300">Quản trị <span className="text-indigo-600">SaveFood</span></span>
            )}
          </div>
          <button 
        ...
        {/* Navigation */}
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1 overflow-x-hidden">
          {!isDesktopCollapsed && <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 px-2">Quản lý</div>}
          {navItems.map((item) => {
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    {isActive && !isDesktopCollapsed && <ChevronRight className="w-4 h-4 ml-auto text-indigo-600 shrink-0" />}
                  </>
                )}
              </NavLink>
            );
        ...
        <div className="p-4 border-t border-gray-100 flex flex-col gap-2">
          ...
          <div 
            ...
            className={cn(
              "flex items-center p-3 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors group",
              isDesktopCollapsed ? "justify-center" : "gap-3"
            )}
            title={isDesktopCollapsed ? (user?.fullName || 'Quản trị viên') : undefined}
          >
          ...
          <button 
            ...
            className={cn(
              "flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-xl transition-colors",
              isDesktopCollapsed && "px-0"
            )}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center px-4 sticky top-0 z-30">
          ...
          <button 
            ...
            className="hidden lg:flex p-2 -ml-2 text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-100"
          >
